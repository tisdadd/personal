...
test('Can navigate pages', async () => {
  ...
  expect(await window.getByText('Pick A CSV to Create or Add to Table').count()).toBe(1);
  expect(await window.getByText(/learn react/i).count()).toBe(0);

  await window.getByText(/See Default React View/i).click();

  expect(await window.getByText(/learn react/i).count()).toBe(1);
